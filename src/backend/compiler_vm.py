...
class CompilerVM:
    ...
    def __init__(self, src_path: str, workspace: str, compiler_option_path: str) -> None:
        ...
        self._DEF_MAKER_DICT: dict[str, Callable[[list[str]], definition.Definition]] = {
            ...
            "C_PART_SQ": lambda cmd: self.__make(definition.CPartSqDef(self._src_info, self._symbol_table.namespace,
                                                                       ...
                                                                       cmd[1:])),
            "CLASS": lambda cmd: self.__make(
                definition.ClassDef(self._src_info, self._symbol_table.namespace, cmd[0], self._var_state_table.state)
            ),
        ...
        self._STMT_MAKER_DICT: dict[str, Callable[[list[str]], statement.Statement]] = {
            ...
            "CONSTRUCTOR_ASSIGN": lambda cmd: self.__make_constructor_assign(),
    ...
    def __call_add_type_arg(self, cmd: list[str]) -> None:
        ...
        # noinspection PyTypeChecker
        t: symbol.TypeName = self._symbol_table[cmd[0]]
        # noinspection PyUnresolvedReferences
    ...
    def __make_constructor_assign(self) -> statement.ConstructorAssignStmt:
        self.__check_type(self._stack[-1], [definition.ConstructorDef])
        # noinspection PyUnresolvedReferences
        stmt = statement.ConstructorAssignStmt(self._src_info, self._stack[-1].this_var)
        return stmt
    ...
    def __make_variable_ref(self, cmd: list[str]) -> expression.VariableRef:
        # noinspection PyTypeChecker
        var_type: symbol.TypeName = self._symbol_table[cmd[0]] if cmd[0] != "auto" else self._symbol_table[cmd[1]]
        var_name: str = cmd[1]
